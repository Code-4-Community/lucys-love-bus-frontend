...
const PersonalRequests: React.FC<PersonalRequestsProps> = ({ personalRequests }) => {
    ...
    const getRoleDescription = (): string | JSX.Element => {
        switch (privilegeLevel) {
            case PrivilegeLevel.STANDARD:
                return (
                    <span>
                        A General Public account can be created by anyone and allows users to sign up for events within 5 days of their start date for $5 per ticket. If you believe you qualify as a Participating Family with full event access you can apply on the {<Link to={Routes.PERSONAL_REQUESTS} component={Typography.Link}> requests page of account settings </Link>}
                    </span>
                )
            case PrivilegeLevel.PF:
                return (
            ...
            case PrivilegeLevel.ADMIN:
                return (
                    <span>
                        An Admin is in charge of creating events and announcements for users of the application. Admin accounts can not be created or deleted but the owners email and password can be changed from the {<Link to={Routes.SETTINGS} component={Typography.Link}> account settings page </Link>}
                    </span>
                )
            default:
                return "";
    ...
    return (
        <>
            {asyncRequestIsFailed(personalRequests) && <p>The request statuses could not be retrieved.</p>}
            {asyncRequestIsLoading(personalRequests) && <p>Loading personal requests...</p>}
                ...
                <>
                    ...
                    <ChungusContentContainer>
                        <Content>
                            ...
                            <StyledSubTitle>
                                Your current account status is: <BoldText>{getUserRole()}</BoldText>
                                <InfoButton onClick={() => { setIsInfoModalVisible((prevState) => !prevState) }}>
                                    ?
                                </InfoButton>
                                <InfoModal
                            ...
                            </StyledSubTitle>
                            {
                                ...
                                <>
                                    ...
                                    <CurrentRequest requests={personalRequests.result} />
                                </>
                            }
                        </Content>
                    </ChungusContentContainer>
